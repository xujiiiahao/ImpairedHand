...
# from utils import earth_mover_distance
...
def inter_penetr_max_loss(hand_xyz, hand_face, obj_xyz, nn_dist, nn_idx):
    """
    Return max penetration distance among all interior object points.
    """
    B = hand_xyz.size(0)
    mesh = Meshes(verts=hand_xyz, faces=hand_face)
    hand_normal = mesh.verts_normals_packed().view(B, -1, 3)

    interior = utils_loss.get_interior(hand_normal, hand_xyz, obj_xyz, nn_idx).bool()

    # 如果没有穿透点，返回 0
    if interior.sum() == 0:
        penetr_max = torch.tensor(0.0, device=hand_xyz.device)
    else:
        penetr_max = nn_dist[interior].max()  # 找最大

    return penetr_max  # 是否乘 100.0 看你训练时是否需要
